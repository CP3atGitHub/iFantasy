...
class Score_APi(Resource):
    ...
    def get(self):
        ...
        pf = data.pf
        c = data.c
        pg_data = query(BagPlayer).filter_by(id=pg).first()
        sg_data = query(BagPlayer).filter_by(id=sg).first()
        sf_data = query(BagPlayer).filter_by(id=sf).first()
        pf_data = query(BagPlayer).filter_by(id=pf).first()
        c_data = query(BagPlayer).filter_by(id=c).first()
...
class OStrategy_RecommendAPi(Resource):
    ...
    def get(self):
        ...
        pf = data.pf
        c = data.c
        if query(SeasonData).filter_by(id=pg).first() is None or query(SeasonData).filter_by(id=sg).first() is None \
                or query(SeasonData).filter_by(id=sf).first() is None or query(SeasonData).filter_by(
            id=c).first() is None \
                or query(SeasonData).filter_by(id=c).first() is None:
            return TacMessage(None, *TacError.No_Data).response
...
class DStrategy_RecommendAPi(Resource):
    ...
    def get(self):
        ...
        if query(SeasonData).filter_by(player_id=pg).first() is None or query(SeasonData).filter_by(
            ...
            if query(PlayerBase).filter_by(player_id=pg).first() is None or query(PlayerBase).filter_by(
                    player_id=sg).first() \
